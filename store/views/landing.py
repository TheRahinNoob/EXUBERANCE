...
from store.models.landing import FeaturedCategory, HotCategory, HotCategoryBlock, HotCategoryBlockItem
...
class LandingHeroBannerAPIView(APIView):
    ...
    def get(self, request):
        banners = (
            ...
            .only("id", "image_desktop", "image_tablet", "image_mobile", "ordering")
...
class LandingFeaturedCategoriesAPIView(APIView):
    ...
    def get(self, request):
        items = (
            ...
            .only("ordering", "image", "category__name", "category__slug")
...
# ==================================================
# LANDING — HOT CATEGORIES (BLOCK-RESPECTING)
# ==================================================
class LandingHotCategoriesAPIView(APIView):
    permission_classes = [AllowAny]

    def get(self, request):
        # Prefetch the active HotCategoryBlockItems and related HotCategory -> Category
        blocks = (
            ...
            .prefetch_related(
                Prefetch(
                    "items",
                    queryset=HotCategoryBlockItem.objects.filter(is_active=True)
                    .select_related("hot_category__category")
                    .order_by("ordering", "id"),
            ...
            hot_categories = [
                {
                    ...
                    "image": request.build_absolute_uri(item.hot_category.image.url)
                    if item.hot_category.image else None,
                }
                for item in block.active_items
            ]
...
class LandingComfortEditorialAPIView(APIView):
    ...
    def get(self, request):
        blocks = ComfortEditorialBlock.objects.filter(is_active=True).order_by("ordering", "id")
    ...
    def get(self, request):
        rails = (
            ...
            .prefetch_related(
                Prefetch(
                    "products",
                    queryset=ComfortCategoryRail.products.rel.model.objects.filter(is_active=True)
                    .only("id", "name", "slug", "price", "old_price", "main_image"),
                )
            )
            .order_by("ordering", "id")
        )
        ...
        for rail in rails:
            ...
            items.append({
                ...
                "category": {
                    ...
                    "image": request.build_absolute_uri(rail.image.url) if rail.image else None,
                },
                "products": ProductMiniSerializer(products, many=True, context={"request": request}).data,
            })
